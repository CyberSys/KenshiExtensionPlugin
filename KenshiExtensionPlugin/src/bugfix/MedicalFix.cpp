...
namespace
{
	...
	void MedicalSystem_updateDamageState_hook(MedicalSystem* self)
	{
		...
		if (KEP::settings._fixMovementAnimation)
	...
	{
		if (!KEP::settings._fixPetRecoverySpeed)
		{
		...
		if (!self->dead)
		{
			...
			float degeneration = KEP::lerp(self->stats->_toughness * self->stats->ageMult * 0.01f, con->DEGENERATION_MULT_1, con->DEGENERATION_MULT_99);
	...
	{
		if (!KEP::settings._fixHPBarOrder)
		{
		...
		auto myRace = self->me->getRace();
		self->blood = KEP::lerp(self->me->getAge(), myRace->originalBloodMin, myRace->originalBloodMax);
		if (myRace->robot)
			self->hunger = 3.0f;
	}
	...
	{
		if (!KEP::settings._fixTheInjuryCalculation)
		{
